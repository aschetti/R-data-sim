...
opts_chunk$set(
  ...
  fig.dim = c(6, 6) # figure width & height
)
...
# parameters
between_ssj <- list(condition = c("test")) # list of between-subject factors
n_ssj <- 50 # number of samples (here, participants)
mu_cond <- 1 # variable means
sd_cond <- 1 # variable standard deviations
...
ttest_onesample_data <-
  sim_design(
    ...
    id = list(participant = "participant"),  # name of ID column
    plot = FALSE, # plot
...
ttest_between_data <-
  sim_design(
    between = list(treatment = c("control", "drug")),
    n = c(50, 50),
    mu = c(1, 2),
    sd = c(1, 1.2),
    empirical = FALSE,
    long = TRUE,
    dv = list(value = "value"),
    id = list(participant = "participant"),
    plot = FALSE,
    rep = 1
  ) %>% 
  as_tibble()
...
ttest_between_results <-
  t.test(
    x = filter(ttest_between_data, treatment == "control")$value,
    y = filter(ttest_between_data, treatment == "drug")$value,
    alternative = "two.sided",
    mu = 0,
    paired = FALSE,
    var.equal = FALSE,
...
my_comparisons <- list(c("condition", 0))
...
ttest_within_results <-
  t.test(
    x = filter(ttest_within_data, condition == "neutral")$value,
    y = filter(ttest_within_data, condition == "emotion")$value,
    alternative = "two.sided",
    mu = 0,
    paired = TRUE,
    var.equal = FALSE,
