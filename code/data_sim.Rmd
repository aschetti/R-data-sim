...
- R packages:
  ...
  - [`tidyverse`](https://github.com/tidyverse/tidyverse), for data wrangling
...
Imagine we show to one group of participants emotional or neutral pictures before the online test. All participants (in this case, N = 50) are presented with all pictures, so this is a within-subject design. Based on published literature and/or our own data collected as part of previous projects, we expect the test scores to follow:
...
`faux` also allows to simulate data similar to an already existing dataset. Let's try and simulate a dataset similar to what we generated for our within-subject t-test example. First, we load the original data:
...
synth_ttest_within_data <- 
  sim_df(
    original_ttest_within_data,
    n = 50, # number of samples per group (can be equal to the original or different)
...
synth_ttest_within_plot <-
  synth_ttest_within_data %>%
  plot_design(geoms = c("box", "violin", "jitter")) +
  ggtitle("Synthetic data, within-subject t-test") +
  theme_custom
...
many simulation packages in R; here are some others. 


- simstudy: Simulation of Study Data
- simr: Power Analysis of Generalized Linear Mixed Models by Simulation
