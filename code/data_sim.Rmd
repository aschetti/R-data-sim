...
Make sure you have [R](https://cran.r-project.org/) and [Rstudio](https://rstudio.com/products/rstudio/download/) installed on your computer and fully functional.

Next, install and load the following R packages:
...
There are many ways to simulate data in R. The easiest way is to use base R functions. Here we use `rnorm` to generate 1000 random samples from a normal distribution with $\mu$ = 0 and $\sigma$ = 1:
...
Let's plot the simulated data. We expect the majority of generated samples to be clustered around 0 (because `mean = 0`), but a few samples can still be larger or smaller than the mean.
...
Now we have all the parameters we need to generate (hypothetical) scores of this test, which will follow the distribution specified above. The relevant function in the `faux` package is `sim_design`:
...
Our research hypothesis is that the test scores will be significantly larger than 0. We can test this hypothesis in several ways, but we keep it simple and run a one-sample Students' t-test against $\mu$ = 0:
