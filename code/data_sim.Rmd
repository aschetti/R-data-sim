...
Our research hypothesis is that the test scores will be statistically larger after exposure to the emotional relative to neutral pictures. Let's run a one-tailed paired-sample Student's t-test:
...
ANOVA_param_recovery <-
  ...
  ungroup() %>% 
  mutate(
    mean_imputed = map_chr(ANOVA_within, toString),
    sd_imputed = map_chr(ttest_within_sd_cond, toString)
  ) %>% 
  select(variable, condition, n, mean, mean_imputed, sd, sd_imputed)
